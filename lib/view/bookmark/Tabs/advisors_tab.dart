...
import 'package:pachub/models/unbookmarkmodel.dart';
...
class AdvisorsTab extends StatefulWidget {
  const AdvisorsTab({Key? key}) : super(key: key);
...
class _AdvisorsTabState extends State<AdvisorsTab> {
  ...
  void initState() {
    ...
    super.initState();
    specialliBookMarkList(_page, _limit);
  }

  @override
  Widget build(BuildContext context) {
    print("list data ===>>> $speciallistsList");
      ...
      child: Padding(
        ...
        Align(
          ...
          child: CustomLoader(),
            ...
            : speciallistsList.isNotEmpty ?
        ListView.builder(
          shrinkWrap: true,
          itemCount: speciallistsList.length,
          itemBuilder: (context, index) {
            var item = speciallistsList[index];
            return  Card(
              key: UniqueKey(),
                              ...
                              child: Column(
                                ...
                                ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      uniquelist.add(itemList[index]["displayGroup"]);
  ...
  Future<bool> _BoookmarkDialog(String userId) async {
    final shouldPop = await showDialog(
      context: context,
      builder: (context) => Padding(
  ...
  specialliBookMarkList(int page, int limit) async {
    log('this is Manage_Profile  api call', name: "Manage_Profile");
    setState(() {
      _isLoadRunning = true;
    });
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          speciallistsList = group3["Advisor"];
          _isLoadRunning = false;
          print("+++++Item ${speciallistsList}");
        });
      } else {
  ...
  Unbookmarklistapi(String userId) async {
    setState(() {
      _isLoadRunning = true;
    });
    try {
      ...
      print("$params");
      var response = await dio.post(AppConstant.BookMarkListTICK,
          data: params,
          options: Options(
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          print("bookmarklist click Response:" + response.data["Message"]);
